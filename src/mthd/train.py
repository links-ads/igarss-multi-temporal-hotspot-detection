...
argparser.add_argument(
    ...
    default=2,
    help=
    "Each dynamic world instance we be weighted by this amount relative to each eo instance",
)
argparser.add_argument("--batch_size", type=int, default=4096)
argparser.add_argument("--dataloader_length",
                       type=int,
                       default=5950,
                       help="-1 to re-estimate dataloader length")
argparser.add_argument(
    "--mask_strategies",
    type=str,
    default=[
...
argparser.add_argument(
    ...
    type=str,
    default=
    f"gs://{TAR_BUCKET}/S1_S2_ERA5_SRTM_2020_2021_DynamicWorldMonthly2020_2021_tars/"
...
argparser.add_argument("--skip_finetuning",
                       dest="skip_finetuning",
                       action="store_true")
...
output_parent_dir = Path(
    args["output_dir"]) if args["output_dir"] else Path(__file__).parent
run_id = None
if wandb_enabled:
    import wandb
...
if (batch_size != argparser.get_default("batch_size")) & (
    ...
    warnings.warn("Dataloader length calculated for a specific batch size. "
...
def load_dataset(url, shuffle_on_load):
    dataset = S1_S2_ERA5_SRTM_DynamicWorldMonthly_2020_2021(
...
if cropharvest_per_n_validations != 0:
    cropharvest_validation = CropHarvestMultiClassValidation(
        n_per_class=cropharvest_val_n_per_class
        if cropharvest_val_n_per_class != -1 else None)
...
if wandb_enabled:
    ...
    start_months_tensor = torch.stack(
        [torch.tensor(ex.start_month).to(device) for ex in examples])
...
with tqdm(range(num_epochs), desc="Epoch") as tqdm_epoch:
    for epoch in tqdm_epoch:
        ...
        train_size = 0
        model.train()
        for epoch_step, b in enumerate(
                tqdm(train_dataloader, desc="Train", leave=False)):
            mask, x, y, start_month = b[0].to(device), b[2].to(
                device), b[3].to(device), b[6]
            dw_mask, x_dw, y_dw = b[1].to(device), b[4].to(
                device).long(), b[5].to(device).long()
            ...
            num_eo_masked, num_dw_masked = len(y_pred[mask]), len(
                dw_pred[dw_mask])
            with torch.no_grad():
            ...
            if training_step % val_per_n_steps == 0:
                ...
                # ------------------------ Metrics + Logging -------------------------------
                if ((cropharvest_per_n_validations != 0) and
                    (num_validations % cropharvest_per_n_validations == 0)
                        and wandb_enabled):
                ...
                train_eo_loss = total_eo_train_loss / max(
                    total_num_eo_values_masked, 1)
                train_dw_loss = total_dw_train_loss / max(
                    total_num_dw_values_masked, 1)

                val_loss = total_val_loss / num_val_updates_captured
                val_eo_loss = total_eo_val_loss / max(
                    total_val_num_eo_values_masked, 1)
                val_dw_loss = total_dw_val_loss / max(
...
if not skip_finetuning:
    ...
    eval_task_list: List[EvalTask] = [
        *[
            CropHarvestEval("Kenya", ignore_dynamic_world=True, seeds=[s])
            for s in eval_seeds
        ],
        *[
            CropHarvestEval("Togo", ignore_dynamic_world=True, seeds=[s])
            for s in eval_seeds
        ],
        *[
            CropHarvestEval("Brazil", ignore_dynamic_world=True, seeds=[s])
            for s in eval_seeds
        ],
    ...
    for seed in eval_seeds:
        eval_task_list.extend([
            CropHarvestEval("Togo",
                            ignore_dynamic_world=True,
                            num_timesteps=x,
                            seeds=[seed]) for x in range(1, 12)
        ])
        eval_task_list.extend([
    ...
    for eval_task in tqdm(eval_task_list, desc="Full Evaluation"):
        ...
        if "EuroSat" in eval_task.name:
            model_modes = [
                "Regression", "Random Forest", "KNNat5", "KNNat20", "KNNat100"
            ]
